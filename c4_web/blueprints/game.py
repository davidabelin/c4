...
from c4_core.forecast import forecast_columns, terminal_outcome_for_board
...
def analyze_game(game_id: int):
    ...
    board = _decode_board(game.get("current_board_json", "[]"))
    lookahead = max(1, min(6, int(request.args.get("lookahead", 4))))
    ...
    try:
        forecasts = forecast_columns(board, perspective_mark=1, lookahead=lookahead, config=_config())
        ...
        return jsonify(
            {
                "analysis": {
                    ...
                    "note": "Estimated likelihood choosing a column will result in a win for the player. These percentages are independent estimates and do not sum to 100%.",
                    "measure": "independent_win_estimate",
                }
            }
        )
